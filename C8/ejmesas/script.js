...
noticias.forEach(x=>{
    const noticia= `<div class="col-sm-6"><div class="card" style="width: 505px">  
    <div>${x.fecha}</div>
    <div>${x.tipoNacional? "Nacional":"Internacional"}</div>
    <div><img src="${x.imgUrl}" class="card-img-top" style="width: 500px"></div>
    <h4>${x.titulo}</h4>
    <p class="card-text">${x.descripcion}</p>
    </div></div>`
    contenedor.innerHTML+= noticia
    console.log(x)
})
